...
SciPy and scikit-learn have a library-specific Python API for controlling parallelism. SciPy's `workers` can mean multi-threading, multiprocessing, or pthreads. Scikit-learn's `n_jobs` is either multiprocessing or multi-threading. Note that scikit-learn's `n_jobs` does not configure OpenMP or OpenBLAS parallelism.
...
2. Adopt a consistent Python API for configuring parallelism. We use SciPy's `workers` parameter because it is more consistent in controlling the number of cores used. `workers` denotes any form of parallalism such as: multi-threading, multiprocessing, OpenMP threads, or pthreads. A library with nested parallel calls will configure the computational routines to prevent subscription. For example, if multiple OpenMP threads call BLAS routines, then BLAS is configured to run serially.
