...
Welcome to the Design Document for CPU parallelism in NumPy, SciPy, scikit-learn, and pandas. Each library has varying levels of support for running parallel computation. This document details the current status of parallelism with shipping code on PyPI versus shipping code on conda-forge and possible paths for improvement.

# Current Landscape

Each library ships with an assortment of parallel interfaces on PyPi:

- NumPy's `linalg` module and matrix multiplication utilize BLAS, which is multi-threaded by default on most implementations. On PyPI NumPy ships with OpenBLAS built with pthreads.
...
On conda-forge, if a library requires OpenMP or an implementation of BLAS (such as MKL or OpenBLAS), it depends on the package distributing the shared library:
...
- There are performance issues when two OpenBLAS are present, such when NumPy and SciPy are both installed via PyPI: [scipy#15129](https://github.com/scipy/scipy/issues/15129). The current workaround is to set `OPENBLAS_THREAD_TIMEOUT=1` on the affected platforms.
