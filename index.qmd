...
- Adopt a consistent Python API for configuring parallelism. I propose using SciPy's `workers` parameter, because it is more consisntet in controlling the number of cores used. Please see the [technical details section](#which-api-to-use for-parallalism) on how this could work.
...
## Which API to use for parallalism?

Currently, SciPy's `workers` parameter is more consistent compared to Scikit-learn's `n_jobs` parameter whne it comes to controlling the number of CPU cores to use. Scikit-learn's `n_jobs` does not configure multithreading when using OpenMP or OpenBLAS.
