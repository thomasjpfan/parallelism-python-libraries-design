...
On PyPi, if a library requires OpenMP or OpenBLAS, it bundles the shared library into the wheel: [Source](https://www.slideshare.net/RalfGommers/parallelism-in-a-numpybased-program)

![Current PyPI landscape](images/pypi.jpg){fig-alt="NumPy, SciPy, and Scikit-learn vendoring shared libraries such as OpenMP or OpenBLAS"}
...
- `libgomp` (OpenMP for GCC) is not fork-safe while libomp (OpenMP for Clang) is fork-safe. Scikit-learn has developed [loky](https://github.com/joblib/loky)  as a workaround. There is a [patch to GCC OpenMP](https://gcc.gnu.org/bugzilla/show_bug.cgi?id=60035) to make it fork safe, but it has not progressed. For details, see [this FAQ entry](https://scikit-learn.org/stable/faq.html#why-do-i-sometime-get-a-crash-freeze-with-n-jobs-1-under-osx-or-linux).
- `libomp` (OpenMP for Clang) not compatible with libiomp (OpenMP for Intel Complier). The workaround is to set MKL_THREADING_LAYER=GNU. See [this link](https://github.com/joblib/threadpoolctl/blob/master/multiple_openmp.md) for details.
...
- Move towards a single OpenMP and OpenBLAS on PyPI by shipping an OpenMP and OpenBLAS wheel.NumPy, SciPy, and Scikit-learn will link to those libraries during runtime. Please see the [technical details section](#technical-details) for details on how this could work.
...
libgomp is not fork safe, but uses the GCC. We advocate for the patch into GCC to make it fork safe.
libomp is fork safe, but it is an implementation detail and not part of the OpenMP specification.
I would go with libomp, because it has the same symbols as libgomp and is fork safe. Upstream libraries such as NumPy or SciPy can still use GCC as their compiler.

### What does conda-forge do?
For BLAS, conda-forge builds with netlib. During installation time, BLAS can be switched to other implementations such as MKL, BLIS, OpenBLAS. See [this link](https://conda-forge.org/docs/maintainer/knowledge_base.html#switching-blas-implementation) for details.

For OpenMP, conda-forge builds with libgomp, the GNU build of OpenMP. During installation time, OpenMP can be switched to libomp, the LLVM build of OpenMP. Recall that the LLVM implementation is fork-safe. Note, that the GNU implementation has target offloading symbols, while LLVM does not. See this [link](https://conda-forge.org/docs/maintainer/knowledge_base.html#openmp) for details.
